...
def select_option(prompt: str, options: List[str], default: int = 0) -> str:
    ...
    if not sys.stdin.isatty() or os.environ.get("CI", "false").lower() == "true":
        print(
            f"Non-interactive mode detected; defaulting to option {default+1} ({options[default]})"
        )
...
def main() -> None:
    ...
    query = (
        f"armRegionName eq '{selected_region}' and "
    ...
    print(
        tabulate(
            [table_data[0]] + sorted_table_data, headers="firstrow", tablefmt="psql"
        )
    )
